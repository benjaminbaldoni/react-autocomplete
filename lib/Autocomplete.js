...
class Autocomplete extends React.Component {
  ...
  static propTypes = {
    ...
    /**
     * Turn on/off the multiple select behaviour
     */
    multiple: PropTypes.bool,
    /**
     * Determine whether the menu should be closed after an item has been selected, or not
     */
  ...
  static defaultProps = {
    ...
    renderMenu(items, value, style) {
      return <div className="Dropdown__Menu" style={{ ...style, ...this.menuStyle }} children={items}/>
    },
    menuStyle: {
      background: '#fff',
      position: 'fixed',
  ...
  handleChange(event) {
    ...
    const highlightedIndex = this.props.items.findIndex(
  ...
  selectItem(item) {
    ...
    const isOpen = !this.props.closeOnSelect;
  ...
  renderItem(item, itemIndex) {
    const isHighlighted = itemIndex == this.state.highlightedIndex;
    const isSelected = this.state.selectedKeys[item[this.props.itemsKey]];
    
    const highlightedClass = (isHighlighted) ? 'Dropdown__Item--highlighted' : '';
    const selectedClass = (isSelected) ? 'Dropdown__Item--selected' : '';

    return (
      <div className={`Dropdown__Item ${highlightedClass} ${selectedClass}`}>
        {this.props.renderItem(item, isHighlighted, isSelected)}
      </div>
    )
  }
  ...
  render() {
    ...
    return (
      <div 
        ...
        className={this.state.isOpen ? 'Dropdown--open' : ''}
      >
        <div>
          {this.renderButton()}
