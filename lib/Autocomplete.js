...
class Autocomplete extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      searchValue: '',
      selection: (this.props.multiple) ? [] : null,
    }
    this._debugStates = [];
  ...
  selectItem(item) {
    const { selection, selected }  = this.handleItemSelection(item);
    ...
    this.setState({
      ...
      if (this.props.multiple) {
        // Calculate entire selection
        const currentSelection = this.props.items.filter(
          item => this.state.selection[item[this.props.itemsKey]]
        );

        this.props.onSelect(item, currentSelection, selected);
      } else {
  ...
  handleItemSelection(item) {
    if (this.props.multiple) {
      ...
      const isAlreadySelected = currentSelection[itemKey];

      currentSelection[itemKey] = !isAlreadySelected;
  ...
  maybeAutoCompleteText() {
    if (!this.props.autoHighlight || this.state.searchValue === '')
      return
    const { highlightedIndex } = this.state
  ...
  renderMenu() {
    const items = this.getFilteredItems().map((item, index) => {      
      const element = this.renderItem(item, index === this.state.highlightedIndex);
  ...
  renderItem(item, isHighlighted) {
    return (
      <div
        className={`Dopdown__Item ${isHighlighted ? 'Dopdown__Item--highlighted' : ''}`}
      >
